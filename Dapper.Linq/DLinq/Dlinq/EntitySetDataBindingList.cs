...
namespace System.Data.Linq {
    internal class EntitySetBindingList<TEntity> : SortableBindingList<TEntity>
        ...
        internal EntitySetBindingList(IList<TEntity> sequence, EntitySet<TEntity> data)
            : base(sequence) {
            if (sequence == null) {
                 throw new Exception(); //ArgumentNull("sequence");
            }
            if (data == null) {
        ...
        private void ThrowEntitySetErrorsIfTypeInappropriate() {
            ...
            if (type.IsAbstract) {
                 throw new Exception(); //EntitySetDataBindingWithAbstractBaseClass(type.Name);
            }
            if (type.GetConstructor(System.Type.EmptyTypes) == null) {
                 throw new Exception(); //EntitySetDataBindingWithNonPublicDefaultConstructor(type.Name);
            }
        }
