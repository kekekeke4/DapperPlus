...
namespace System.Data.Linq {
    sealed public class DataLoadOptions {
        ...
        internal void Preload(MemberInfo association) {
            if (association == null) {
                 throw new Exception(); //ArgumentNull("association");
            }
            if (this.frozen) {
                 throw new Exception(); //IncludeNotAllowedAfterFreeze();
            }
        ...
        private void Subquery(MemberInfo association, LambdaExpression subquery) {
            if (this.frozen) {
                 throw new Exception(); //SubqueryNotAllowedAfterFreeze();
            }
        ...
        private static MemberInfo GetLoadWithMemberInfo(LambdaExpression lambda)
        {
            ...
            {
                 throw new Exception(); //InvalidLoadOptionsLoadMemberSpecification();
            }
        }
        ...
        private void ValidateTypeGraphAcyclic() {
            ...
            for (int loop = 0; loop < this.includes.Count; ++loop) {
                ...
                if (!someRemoved) {
                     throw new Exception(); //IncludeCycleNotAllowed(); // No edges removed, there must be a loop.
                }
                edges = newEdges;
            }
        ...
        private static void ValidateSubqueryMember(MemberInfo mi) {
            ...
            if (memberType == null) {
                 throw new Exception(); //SubqueryNotSupportedOn(mi);
            }
            if (!typeof(IEnumerable).IsAssignableFrom(memberType)) {
                 throw new Exception(); //SubqueryNotSupportedOnType(mi.Name, mi.DeclaringType);
            }
        }
        ...
        private static void ValidateSubqueryExpression(LambdaExpression subquery) {
            if (!typeof(IEnumerable).IsAssignableFrom(subquery.Body.Type)) {
                 throw new Exception(); //SubqueryMustBeSequence();
            }
            ...
            internal override Expression VisitMethodCall(MethodCallExpression m) {
                ...
                try {
                    if (isTopLevel && !SubqueryRules.IsSupportedTopLevelMethod(m.Method))
                         throw new Exception(); //SubqueryDoesNotSupportOperator(m.Method.Name);
                    isTopLevel = false;
