    ...
    internal class SqlResolver {
        ...
        class Visitor : SqlScopedVisitor {
            ...
            internal override SqlExpression VisitColumnRef(SqlColumnRef cref) {
                ...
                if (result == null) {
                     throw new Exception(); //ColumnReferencedIsNotInScope(GetColumnName(cref.Column));
                }
                return result;
            }
            ...
            internal override SqlRow VisitRow(SqlRow row) {
                foreach (SqlColumn c in row.Columns) {
                    if (this.RefersToColumn(c, this.match)) {
                        if (this.found != null) {
                             throw new Exception(); //ColumnIsDefinedInMultiplePlaces(GetColumnName(this.match));
                        }
                        this.found = c;
                        break;
                    }
                ...
                if (this.found == null) {
                    ...
                    if (this.found != null) {
                        if (select.IsDistinct && !match.IsConstantColumn) {
                             throw new Exception(); //ColumnIsNotAccessibleThroughDistinct(GetColumnName(this.match));
                        }
                        if (select.GroupBy.Count == 0 || this.IsFoundInGroup(select)) {
                            ...
                            // found it, but its hidden behind the group-by
                             throw new Exception(); //ColumnIsNotAccessibleThroughGroupBy(GetColumnName(this.match));
                        }
                    }
                }
