...
namespace System.Data.Linq.SqlClient {
    ...
    internal class QueryConverter {
        ...
        internal QueryConverter(IDataServices services, TypeSystemProvider typeProvider, Translator translator, SqlFactory sql) {
            if (services == null) {
                 throw new Exception(); //ArgumentNull("services");
            }
            if (sql == null) {
                 throw new Exception(); //ArgumentNull("sql");
            }
            if (translator == null) {
                 throw new Exception(); //ArgumentNull("translator");
            }
            if (typeProvider == null) {
                 throw new Exception(); //ArgumentNull("typeProvider");
            }
        ...
        internal SqlNode ConvertOuter(Expression node) {
            ...
            if (retNode.NodeType == SqlNodeType.MethodCall) {
                ...
                // must be either a mapped stored procedure or a mapped function
                 throw new Exception(); //InvalidMethodExecution(((SqlMethodCall)retNode).Method.Name);
            }
                ...
                switch (node.NodeType) {
                    ...
                    case ExpressionType.UnaryPlus:
                        if (node.Type == typeof(TimeSpan))
                            return this.VisitUnary((UnaryExpression)node);
                         throw new Exception(); //UnrecognizedExpressionNode(node.NodeType);
                    ...
                    case ExpressionType.LeftShift:
                         throw new Exception(); //UnsupportedNodeType(node.NodeType);
                ...
                if (node.NodeType == SqlNodeType.Value) {
                    ...
                    if (query != null) {
                        ...
                        if (fex.NodeType != ExpressionType.Constant ||
                         ...
                         throw new Exception(); //IQueryableCannotReturnSelfReferencingConstantExpression();
                    }
                     throw new Exception(); //CapturedValuesCannotBeSequences();
                }
                    ...
                    return ((SqlSubSelect)node).Select;
                     ...
                     throw new Exception(); //ConstructedArraysNotSupported();
                }
                else if (node.NodeType == SqlNodeType.ClientParameter) {
                     throw new Exception(); //ParametersCannotBeSequences();
                }
        ...
        private SqlNode VisitLambda(LambdaExpression lambda) {
            ...
            for (int i = 0, n = lambda.Parameters.Count; i < n; i++) {
                ...
                if (p.Type == typeof(Type)) {
                     throw new Exception(); //BadParameterType(p.Type);
                }
...
#pragma warning restore 618
        ...
        private SqlExpression VisitExpression(Expression exp) {
            ...
            if (select != null) {
             ...
             throw new Exception(); //UnrecognizedExpressionNode(result);
        }
            ...
            if (pselect != null) {
                ...
                if (expr != null) {
                    return new SqlSelect(expr, alias, this.dominatingExpression);
                     ...
                     throw new Exception(); //BadProjectionInSelect();
                }
            }
        }
        ...
        // Leo
        private SqlSelect VisitJoin(Expression outerSequence, Expression innerSequence, ConstantExpression joinTypeSelector, LambdaExpression predicateSelector, LambdaExpression resultSelector)
        {
            ...
            SqlJoinType joinType = (SqlJoinType)joinTypeSelector.Value;
            this.map[predicateSelector.Parameters[0]] = outerRef;
            this.map[predicateSelector.Parameters[1]] = innerRef;
            SqlExpression condition = this.VisitExpression(predicateSelector);
            ...
            SqlJoin join = new SqlJoin(joinType, outerAlias, innerAlias, condition, this.dominatingExpression);
            SqlSelect select = new SqlSelect(result, join, this.dominatingExpression);
            return select;
        }
        ...
        private SqlNode VisitTypeBinary(TypeBinaryExpression b) {
            ...
            switch (b.NodeType) {
                case ExpressionType.TypeIs:
                    ...
                    break;
                default:
                     throw new Exception(); //TypeBinaryOperatorNotRecognized();
            }
            return result;
        }
        ...
        private SqlNode VisitAs(UnaryExpression a) {
            ...
            if (select != null) {
             ...
             throw new Exception(); //DidNotExpectAs(a);
        }
...
#pragma warning restore 618
             ...
             throw new Exception(); //UnrecognizedExpressionNode(b.NodeType);
        }
        ...
        private SqlNode ChangeType(SqlExpression expr, Type type) {
            ...
            switch (cm) {
                ...
                default:
                     throw new Exception(); //UnhandledExpressionType(cm);
            }
        }
        ...
        private SqlNode TranslateConstantTable(ITable table, SqlLink link) {
            if (table.Context != this.services.Context) {
                 throw new Exception(); //WrongDataContext();
            }
        ...
        private SqlExpression VisitNew(NewExpression qn) {
            ...
            if (mt.IsEntity) {
                 throw new Exception(); //CannotMaterializeEntityType(qn.Type);
            }
        ...
        private SqlExpression VisitMemberInit(MemberInitExpression init) {
            ...
            if (mt.IsEntity) {
                 throw new Exception(); //CannotMaterializeEntityType(init.Type);
            }
            SqlExpression[] args = null;
            ...
            for (int i = 0; i < cBindings; i++) {
                ...
                if (mb != null) {
                    ...
                    ordinal[i] = mt.GetDataMember(mb.Member).Ordinal;
                     ...
                     throw new Exception(); //UnhandledBindingType(init.Bindings[i].BindingType);
                }
            }
        ...
        private static IEnumerable<MemberInfo> PropertyOrFieldOf(IEnumerable<MemberInfo> members) {
            ...
            foreach (MemberInfo mi in members) {
                switch (mi.MemberType) {
                    ...
                    default: {
                             throw new Exception(); //CouldNotConvertToPropertyOrField(mi);
                    }
                }
            }
            return result;
        ...
        private SqlSelect VisitTake(Expression sequence, Expression count) {
            ...
            if (takeExp.NodeType == SqlNodeType.Value) {
                ...
                if (typeof(int).IsAssignableFrom(constTakeCount.Value.GetType()) && ((int)constTakeCount.Value) < 0) {
                     throw new Exception(); //ArgumentOutOfRange("takeCount");
                }
            }
            ...
            if (mce != null && IsSequenceOperatorCall(mce) && mce.Method.Name == "Skip" && mce.Arguments.Count == 2) {
                ...
                if (skipExp.NodeType == SqlNodeType.Value) {
                    ...
                    if (typeof(int).IsAssignableFrom(constSkipCount.Value.GetType()) && ((int)constSkipCount.Value) < 0) {
                         throw new Exception(); //ArgumentOutOfRange("skipCount");
                    }
                }
        ...
        private SqlSelect GenerateSkipTake(SqlSelect sequence, SqlExpression skipExp, SqlExpression takeExp) {
            ...
            if (this.UseConverterStrategy(ConverterStrategy.SkipWithRowNumber)) {
                ...
                if (!CanSkipOnSelection(select.Selection)) {
                     throw new Exception(); //SkipNotSupportedForSequenceTypes();
                }            
                ...
                stqv.Visit(select);
                if (!stqv.IsValid) {
                     throw new Exception(); //SkipRequiresSingleTableQueryWithPKs();
                }
        ...
        private SqlNode VisitParameter(ParameterExpression p) {
            ...
            if (this.dupMap.TryGetValue(p, out nodeToDup)) {
             ...
             throw new Exception(); //ParameterNotInScope(p.Name);
        }
        ...
        private SqlNode TranslateStoredProcedureCall(MethodCallExpression mce, MetaFunction function) {
            if (!this.outerNode) {
                 throw new Exception(); //SprocsCannotBeComposed();
            }
            ...
            if (returnType.IsGenericType &&
                ...
                || returnType == typeof(int?)
                 ...
                 throw new Exception(); //InvalidReturnFromSproc(returnType);
            }
        ...
        private SqlNode VisitBinary(BinaryExpression b) {
            ...
            switch (b.NodeType) {
                ...
                case ExpressionType.Coalesce:
                    ...
                    break;
                default:
                     throw new Exception(); //BinaryOperatorNotRecognized(b.NodeType);
            }
            return result;
        }
        ...
        private SqlNode VisitIntersect(Expression source1, Expression source2) {
            ...
            if (IsGrouping(type)) {
                 throw new Exception(); //IntersectNotSupportedForHierarchicalTypes();
            }
        ...
        private SqlNode VisitExcept(Expression source1, Expression source2) {
            Type type = TypeSystem.GetElementType(source1.Type);
            if (IsGrouping(type)) {
                 throw new Exception(); //ExceptNotSupportedForHierarchicalTypes();
            }
        ...
        private SqlSelect VisitOrderBy(Expression sequence, LambdaExpression expression, SqlOrderType orderType) {
            if (IsGrouping(expression.Body.Type)) {
                 throw new Exception(); //GroupingNotSupportedAsOrderCriterion();
            }
            if (!this.typeProvider.From(expression.Body.Type).IsOrderable) {
                 throw new Exception(); //TypeCannotBeOrdered(expression.Body.Type);
            }
        ...
        private SqlNode VisitAggregate(Expression sequence, LambdaExpression lambda, SqlNodeType aggType, Type returnType) {
            ...
            if (lambda != null && !TypeSystem.IsSimpleType(lambda.Body.Type)) {
                 throw new Exception(); //CannotAggregateType(lambda.Body.Type);
            }
            ...
            if (select.Selection.SqlType.IsRuntimeOnlyType && !IsGrouping(sequence.Type) && !isCount && lambda == null) {
                 throw new Exception(); //NonCountAggregateFunctionsAreNotValidOnProjections(aggType);
            }
            if (lambda != null)
            ...
            if (mc.Method.IsStatic) {
                ...
                // why is this handled here and not in SqlMethodCallConverter?
                    ...
                    if (mc.Method.Name == "ChangeType") {
                        ...
                        if(sn == null) {
                             throw new Exception(); //MethodFormHasNoSupportConversionToSql(mc.Method.Name, mc.Method);
                        }
                        return sn;
                    }
                }
        ...
        private object GetValue(Expression expression, string operation) {
            ...
            if (exp.NodeType == SqlNodeType.Value) {
             ...
             throw new Exception(); //NonConstantExpressionsNotSupportedFor(operation);
        }
        ...
        private bool IsSequenceOperatorCall(MethodCallExpression mc) {
            ...
            if (declType == typeof(System.Linq.Enumerable) ||
                ...
                declType == typeof(System.Data.Linq.QueryableExtentions)) {
                return true;
            }
            return false;
        }
            ...
            if (IsSequenceOperatorCall(mc)) {
                switch (mc.Method.Name) {
                    ...
                    case "Join":
                        ...
                        if (mc.Arguments.Count == 5)
                        {
                            var arg2 = (ConstantExpression)mc.Arguments[2]; //this.GetLambda(mc.Arguments[2]);
                            var arg3 = this.GetLambda(mc.Arguments[3]);
                            var arg4 = this.GetLambda(mc.Arguments[4]);
                            return this.VisitJoin(mc.Arguments[0], mc.Arguments[1], arg2, arg3,arg4);
                        }
                        break;
                    case "GroupJoin":
                        isSupportedSequenceOperator = true;
                ...
                if (isSupportedSequenceOperator) {
                     throw new Exception(); //QueryOperatorOverloadNotSupported(mc.Method.Name);
                }
                 throw new Exception(); //QueryOperatorNotSupported(mc.Method.Name);
            }
            else {
                 throw new Exception(); //InvalidSequenceOperatorCall(declType);
            }
        }
        ...
        private SqlNode VisitDataManipulationCall(MethodCallExpression mc) {
            if (IsDataManipulationCall(mc)) {
             ...
             throw new Exception(); //InvalidSequenceOperatorCall(mc.Method.Name);
        }
        ...
        private SqlStatement VisitInsert(Expression item, LambdaExpression resultSelector) {
            if (item == null) {
                 throw new Exception(); //ArgumentNull("item");
            }
            ...
            if (conItem == null) {
                 throw new Exception(); //InsertItemMustBeConstant();
            }
        ...
        private SqlExpression GetIdentityExpression(MetaDataMember id, bool isOutputFromInsert) {
            if (isOutputFromInsert) {
                ...
                if (!IsLegalIdentityType(sqlType.GetClosestRuntimeType())) {                    
                     throw new Exception(); //InvalidDbGeneratedType(sqlType.ToQueryString());
                }
        ...
        private SqlStatement VisitUpdate(Expression item, LambdaExpression check, LambdaExpression resultSelector) {
            ...
            try {
                ...
                ConstantExpression conItem = item as ConstantExpression;
                if (conItem == null) {
                     throw new Exception(); //UpdateItemMustBeConstant();
                }
                if (conItem.Value == null) {
                     throw new Exception(); //ArgumentNull("item");
                }
        ...
        private SqlExpression VisitListInit(ListInitExpression list) {
            if (null != list.NewExpression.Constructor && 0 != list.NewExpression.Arguments.Count) {
                ...
                // init does not use a default constructor.
                 throw new Exception(); //UnrecognizedExpressionNode(list.NodeType);
            }
