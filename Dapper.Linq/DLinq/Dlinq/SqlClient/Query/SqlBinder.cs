        ...
        class Visitor : SqlVisitor {
                ...
                switch (bo.NodeType) {
                    ...
                    case SqlNodeType.NE2V:
                        ...
                        if (TypeSystem.IsSequenceType(bo.Left.ClrType)) {
                             throw new Exception(); //ComparisonNotSupportedForType(bo.Left.ClrType);
                        }
                        if (TypeSystem.IsSequenceType(bo.Right.ClrType)) {
                             throw new Exception(); //ComparisonNotSupportedForType(bo.Right.ClrType);
                        }
                        break;
                }
                return bo;
            ...
            private MetaType[] GetPossibleTypes(SqlExpression typeExpression) {
                ...
                if (typeExpression.NodeType == SqlNodeType.DiscriminatedType) {
                 ...
                 throw new Exception(); //UnexpectedNode(typeExpression.NodeType);
            }
                    ...
                    switch (exp.NodeType) {
                        ...
                        case SqlNodeType.TypeCase: {
                                ...
                                foreach (SqlTypeCaseWhen when in tc.Whens) {
                                    ...
                                    if (expr is SqlNew) {
                                         throw new Exception(); //DidNotExpectTypeBinding();
                                    }
                ...
                switch (exp.NodeType) {
                            ...
                            foreach (SqlClientWhen when in sc.Whens) {
                                ...
                                if (newClrType == null) {
                                    newClrType = newValue.ClrType;
                                     ...
                                     throw new Exception(); //ExpectedClrTypesToAgree(newClrType, newValue.ClrType);
                                }
                                matches.Add(when.Match);
                                values.Add(newValue);
                            }
                    ...
                    case SqlNodeType.New: {
                            ...
                            if (!sn.SqlType.CanBeColumn && mm != null) {
                                 throw new Exception(); //MemberNotPartOfProjection(m.Member.DeclaringType, m.Member.Name);
                            }
                            break;
                        }
                    case SqlNodeType.Element:
                    case SqlNodeType.ScalarSubSelect: {
                            ...
                            try {
                                ...
                                if (rselect != null) {
                                 ...
                                 throw new Exception(); //UnexpectedNode(result.NodeType);
                            }
                            finally {
                                this.currentSelect = saveSelect;
                            }
            ...
            private static MetaDataMember GetRequiredInheritanceDataMember(MetaType type, MemberInfo mi) {
                ...
                if (root == null) {
                     throw new Exception(); //UnmappedDataMember(mi, mi.DeclaringType, type);
                }
                    ...
                    case SqlNodeType.ClientCase: {
                            ...
                            foreach (SqlClientWhen when in sc.Whens) {
                                ...
                                if (!sc.ClrType.IsAssignableFrom(when.Value.ClrType)) {
                                     throw new Exception(); //DidNotExpectTypeChange(when.Value.ClrType, sc.ClrType);
                                }
                            ...
                            if (allExprs) {
                                ...
                                for (int i = 0, c = fetchedValues.Count; i < c; ++i) {
                                    ...
                                    if (!cc.ClrType.IsAssignableFrom(fetchedValue.ClrType)) {
                                         throw new Exception(); //DidNotExpectTypeChange(cc.ClrType, fetchedValue.ClrType);
                                    }
            ...
            internal SqlNode ConvertToFetchedSequence(SqlNode node) {
                ...
                if (!TypeSystem.IsSequenceType(expr.ClrType)) {
                     throw new Exception(); //SequenceOperatorsNotSupportedForType(expr.ClrType);
                }

                if (expr.NodeType == SqlNodeType.Value) {
                     throw new Exception(); //QueryOnLocalCollectionNotSupported();
                }
