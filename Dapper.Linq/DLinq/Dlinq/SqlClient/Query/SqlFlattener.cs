            ...
            private void FlattenGroupBy(List<SqlExpression> exprs) {
                ...
                foreach (SqlExpression gex in exprs) {
                    if (TypeSystem.IsSequenceType(gex.ClrType)) {
                         throw new Exception(); //InvalidGroupByExpressionType(gex.ClrType.Name);
                    }
                ...
                if (sn != null) {
                    ...
                    if (expr.NodeType != SqlNodeType.ColumnRef &&
                        ...
                        if (!expr.SqlType.CanBeColumn) {
                             throw new Exception(); //InvalidGroupByExpressionType(expr.SqlType.ToQueryString());
                        }
                         throw new Exception(); //InvalidGroupByExpression();
                    }
                    exprs.Add(expr);
                }
            }
                ...
                foreach (SqlOrderExpression obex in exprs) {
                    if (!obex.Expression.SqlType.IsOrderable) {
                        if (obex.Expression.SqlType.CanBeColumn) {
                             throw new Exception(); //InvalidOrderByExpression(obex.Expression.SqlType.ToQueryString());
                        }
                        else {
                             throw new Exception(); //InvalidOrderByExpression(obex.Expression.ClrType.Name);
                        }
                    }
                }
            }
