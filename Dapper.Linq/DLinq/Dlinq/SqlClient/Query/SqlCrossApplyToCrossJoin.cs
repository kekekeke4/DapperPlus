		...
		class Reducer : SqlVisitor {
			...
			internal override SqlSource VisitJoin(SqlJoin join) {
				if (join.JoinType == SqlJoinType.CrossApply) {
                    ...
                    if (p.Overlaps(c)) {
						Annotations.Add(join, new SqlServerCompatibilityAnnotation("SourceExpressionAnnotation"/*Strings.SourceExpressionAnnotation(join.SourceExpression)*/, SqlProvider.ProviderMode.Sql2000));
