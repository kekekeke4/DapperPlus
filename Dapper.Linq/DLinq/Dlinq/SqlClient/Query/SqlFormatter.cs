    ...
    internal class SqlFormatter : DbFormatter {
        ...
        internal class Visitor : SqlVisitor {
            ...
            internal override SqlExpression VisitNop(SqlNop nop) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("NOP");
                }
                sb.Append("NOP()");
                return nop;
            }
            ...
            internal override SqlExpression VisitUnaryOperator(SqlUnary uo) {
                switch (uo.NodeType) {
                    ...
                    case SqlNodeType.OuterJoinedValue:
                        ...
                        break;
                    default:
                         throw new Exception(); //InvalidFormatNode(uo.NodeType);
                }
                return uo;
            }
            ...
            internal override SqlExpression VisitColumn(SqlColumn c) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("Column");
                }
                sb.Append("COLUMN(");
            ...
            internal override SqlExpression VisitSimpleExpression(SqlSimpleExpression simple) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("SIMPLE");
                }
                sb.Append("SIMPLE(");
            ...
            internal override SqlExpression VisitSharedExpression(SqlSharedExpression shared) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("Shared");
                }
                sb.Append("SHARED(");
            ...
            internal override SqlExpression VisitSharedExpressionRef(SqlSharedExpressionRef sref) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("SharedRef");
                }
                sb.Append("SHAREDREF(");
            ...
            internal override SqlExpression VisitValue(SqlValue value) {
                if (value.IsClientSpecified && !this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("Value");
                }
                else {
            ...
            internal override SqlExpression VisitClientParameter(SqlClientParameter cp) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("ClientParameter");
                }
                else {
            ...
            internal override SqlExpression VisitElement(SqlSubSelect elem) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("Element");
                }
                int saveDepth = this.depth;
                this.depth++;
                sb.Append("ELEMENT(");
            ...
            internal override SqlExpression VisitMultiset(SqlSubSelect sms) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("Multiset");
                }
                int saveDepth = this.depth;
                this.depth++;
                sb.Append("MULTISET(");
            ...
            internal override SqlExpression VisitNew(SqlNew sox) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("New");
                }
                sb.Append("new ");
            ...
            internal override SqlExpression VisitClientArray(SqlClientArray scar) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("ClientArray");
                }
                sb.Append("new []{");
            ...
            internal override SqlSource VisitJoin(SqlJoin join) {
                ...
                switch (join.JoinType) {
                    ...
                    case SqlJoinType.LeftOuter:
                        ...
                        break;
                    case SqlJoinType.RightOuter: // Leo add right sql
            ...
            internal bool RequiresOnCondition(SqlJoinType joinType) {
                switch (joinType) {
                    ...
                    case SqlJoinType.LeftOuter:
                        return true;
                    default:
                         throw new Exception(); //InvalidFormatNode(joinType);
                }
            }
            ...
            internal override SqlExpression VisitClientQuery(SqlClientQuery cq) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("ClientQuery");
                }
                sb.Append("client(");
            ...
            internal override SqlExpression VisitJoinedCollection(SqlJoinedCollection jc) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("JoinedCollection");
                }
                sb.Append("big-join(");
            ...
            internal override SqlExpression VisitClientCase(SqlClientCase c) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("ClientCase");
                }
                this.depth++;
                this.NewLine();
                sb.Append("(CASE");
            ...
            internal override SqlExpression VisitTypeCase(SqlTypeCase c) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("TypeCase");
                }
                this.depth++;
                this.NewLine();
                sb.Append("(CASE");
                ...
                if (value == null) {
                    ...
                    if (this.isDebugMode) {
                        ...
                        sb.Append(")");
                         ...
                         throw new Exception(); //ValueHasNoLiteralInSql(value);
                    }
                }
            }
            ...
            internal virtual string GetOperator(SqlNodeType nt) {
                switch (nt) {
                    ...
                    default:
                         throw new Exception(); //InvalidFormatNode(nt);
                }
            }

            internal override SqlNode VisitLink(SqlLink link) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("Link");
                }
            ...
            internal override SqlMemberAssign VisitMemberAssign(SqlMemberAssign ma) {
                 throw new Exception(); //InvalidFormatNode("MemberAssign");
            }

            internal override SqlExpression VisitMethodCall(SqlMethodCall mc) {
                if (!this.isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("MethodCall");
                }
            ...
            internal override SqlExpression VisitOptionalValue(SqlOptionalValue sov) {
                if (this.isDebugMode) {
                    ...
                    return sov;
                     ...
                     throw new Exception(); //InvalidFormatNode("OptionalValue");
                }
            }

            internal override SqlExpression VisitUserRow(SqlUserRow row) {
                if (!isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("UserRow");
                }
                return row;
            }
            ...
            internal override SqlExpression VisitGrouping(SqlGrouping g) {
                if (!isDebugMode) {
                     throw new Exception(); //InvalidFormatNode("Grouping");
                }
                sb.Append("Group(");
                this.Visit(g.Key);
                sb.Append(", ");
