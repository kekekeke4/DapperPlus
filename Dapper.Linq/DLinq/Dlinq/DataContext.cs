...
namespace System.Data.Linq {
    ...
    public class DataContext : IDisposable {
        ...
        public DataContext(string fileOrServerOrConnection) {
            if (fileOrServerOrConnection == null) {
                 throw new Exception(); //ArgumentNull("fileOrServerOrConnection");
            }
        ...
        public DataContext(string fileOrServerOrConnection, MappingSource mapping) {
            ...
            if (mapping == null) {
                 throw new Exception(); //ArgumentNull("mapping");
            }
        ...
        public DataContext(IDbConnection connection) {
            if (connection == null) {
                 throw new Exception(); //ArgumentNull("connection");
            }
        ...
        internal DataContext(DataContext context) {
            if (context == null) {
                 throw new Exception(); //ArgumentNull("context");
            }
        ...
        internal void CheckDispose() {
            if (this.disposed) {
                 throw new Exception(); //DataContextCannotBeUsedAfterDispose();
            }
        }
        #endregion
        ...
        private void Init(object connection, MappingSource mapping) {
            ...
            if (model.ProviderType != null) {
                providerType = model.ProviderType;
                 ...
                 throw new Exception(); //ProviderTypeNull();
            }

            if (!typeof(IProvider).IsAssignableFrom(providerType)) {
                 throw new Exception(); //ProviderDoesNotImplementRequiredInterface(providerType, typeof(IProvider));
            }
        ...
        public bool ObjectTrackingEnabled {
            ...
            set {
                ...
                if (Services.HasCachedObjects) {
                     throw new Exception(); //OptionsCannotBeModifiedAfterQuery();
                }
        ...
        public bool DeferredLoadingEnabled {
            ...
            set {
                ...
                if (!ObjectTrackingEnabled && value) {
                     throw new Exception(); //DeferredLoadingRequiresObjectTracking();
                }
        ...
        internal void VerifyTrackingEnabled() {
            CheckDispose();
            if (!ObjectTrackingEnabled) {
                 throw new Exception(); //ObjectTrackingRequired();
            }
        }
        ...
        internal void CheckNotInSubmitChanges() {
            ...
            if (this.isInSubmitChanges) {
                 throw new Exception(); //CannotPerformOperationDuringSubmitChanges();
            }
        }
        ...
        internal void CheckInSubmitChanges() {
            CheckDispose();
            if (!this.isInSubmitChanges) {
                 throw new Exception(); //CannotPerformOperationOutsideSubmitChanges();
            }
        }
        ...
        public Table<TEntity> GetTable<TEntity>() where TEntity : class {
            ...
            if (metaTable == null) {
                 throw new Exception(); //TypeIsNotMarkedAsTable(typeof(TEntity));
            }
            ITable table = this.GetTable(metaTable);
            if (table.ElementType != typeof(TEntity)) {
                 throw new Exception(); //CouldNotGetTableForSubtype(typeof(TEntity), metaTable.RowType.Type);
            }
        ...
        public ITable GetTable(Type type) {
            CheckDispose();
            if (type == null) {
                 throw new Exception(); //ArgumentNull("type");
            }
            MetaTable metaTable = this.services.Model.GetTable(type);
            if (metaTable == null) {
                 throw new Exception(); //TypeIsNotMarkedAsTable(type);
            }
            if (metaTable.RowType.Type != type) {
                 throw new Exception(); //CouldNotGetTableForSubtype(type, metaTable.RowType.Type);
            }
        ...
        private static void ValidateTable(MetaTable metaTable) {
            ...
            foreach(MetaAssociation assoc in metaTable.RowType.Associations) {
                if(!assoc.ThisMember.DeclaringType.IsEntity) {
                     throw new Exception(); //NonEntityAssociationMapping(assoc.ThisMember.DeclaringType.Type, assoc.ThisMember.Name, assoc.ThisMember.DeclaringType.Type);
                }
                if(!assoc.OtherType.IsEntity) {
                     throw new Exception(); //NonEntityAssociationMapping(assoc.ThisMember.DeclaringType.Type, assoc.ThisMember.Name, assoc.OtherType.Type);
                }
            }
        }
        ...
        public void Refresh(RefreshMode mode, object entity)
        {
            ...
            if (entity == null)
            {
                 throw new Exception(); //ArgumentNull("entity");
            }
        ...
        public void Refresh(RefreshMode mode, params object[] entities)
        {
            ...
            if (entities == null){
                 throw new Exception(); //ArgumentNull("entities");
            }
        ...
        public void Refresh(RefreshMode mode, IEnumerable entities)
        {
            ...
            foreach (object o in list) {
                ...
                if (trackedObject == null) {
                     throw new Exception(); //UnrecognizedRefreshObject();
                }

                if (trackedObject.IsNew) {
                     throw new Exception(); //RefreshOfNewObject();
                }
                ...
                if (freshInstance == null) {
                     throw new Exception(); //RefreshOfDeletedObject();
                }
        ...
        internal string GetQueryText(IQueryable query) {
            CheckDispose();
            if (query == null) {
                 throw new Exception(); //ArgumentNull("query");
            }
        ...
        public int ExecuteCommand(string command, params object[] parameters) {
            CheckDispose();
            if (command == null) {
                 throw new Exception(); //ArgumentNull("command");
            }
            if (parameters == null) {
                 throw new Exception(); //ArgumentNull("parameters");
            }
        ...
        public IEnumerable ExecuteQuery(Type elementType, string query, params object[] parameters) {
            CheckDispose();
            if (elementType == null) {
                 throw new Exception(); //ArgumentNull("elementType");
            }
            if (query == null) {
                 throw new Exception(); //ArgumentNull("query");
            }
        ...
        internal protected IExecuteResult ExecuteMethodCall(object instance, MethodInfo methodInfo, params object[] parameters) {
            CheckDispose();
            if (instance == null) {
                 throw new Exception(); //ArgumentNull("instance");
            }
            if (methodInfo == null) {
                 throw new Exception(); //ArgumentNull("methodInfo");
            }
            if (parameters == null) {
                 throw new Exception(); //ArgumentNull("parameters");
            }
        ...
        internal protected IQueryable<TResult> CreateMethodCallQuery<TResult>(object instance, MethodInfo methodInfo, params object[] parameters) {
            ...
            if (!typeof(IQueryable<TResult>).IsAssignableFrom(methodInfo.ReturnType)) {
                 throw new Exception(); //ExpectedQueryableArgument("methodInfo", typeof(IQueryable<TResult>));
            }
        ...
        internal protected void ExecuteDynamicInsert(object entity) {
            ...
            if (tracked == null) {
                 throw new Exception(); //CannotPerformOperationForUntrackedObject();
            }
        ...
        public IEnumerable Translate(Type elementType, DbDataReader reader) {
            ...
            if (reader == null) {
                 throw new Exception(); //ArgumentNull("reader");
            }
        ...
        public DataLoadOptions LoadOptions {
            ...
            set {
                ...
                if (this.services.HasCachedObjects && value != this.loadOptions) {
                     throw new Exception(); //LoadOptionsChangeNotAllowedAfterQuery();
                }
                if (value != null) {
                    value.Freeze();
                }
    ...
    public sealed class Table<TEntity> : IQueryable<TEntity>, IQueryProvider, IEnumerable<TEntity>, IQueryable, IEnumerable, ITable, IListSource, ITable<TEntity> 
        ...
        IQueryable IQueryProvider.CreateQuery(Expression expression) {
            if (expression == null) {
                 throw new Exception(); //ArgumentNull("expression");
            }
            ...
            if (!qType.IsAssignableFrom(expression.Type)) {
                 throw new Exception(); //ExpectedQueryableArgument("expression", qType);
            }
        ...
        IQueryable<TResult> IQueryProvider.CreateQuery<TResult>(Expression expression) {
            ...
            if (!typeof(IQueryable<TResult>).IsAssignableFrom(expression.Type)) {
                 throw new Exception(); //ExpectedQueryableArgument("expression", typeof(IEnumerable<TResult>));
            }
        ...
        public void InsertOnSubmit(TEntity entity) {
            ...
            if (!IsTrackableType(type)) {
                 throw new Exception(); //TypeCouldNotBeAdded(type.Type);
            }
            ...
            if (tracked == null) {
                ...
                tracked.ConvertToNew();
                 ...
                 throw new Exception(); //CantAddAlreadyExistingItem();
            }
        }
        ...
        void ITable.InsertOnSubmit(object entity) {
            ...
            if (tEntity == null) {
                 throw new Exception(); //EntityIsTheWrongType();
            }
        ...
        public void DeleteOnSubmit(TEntity entity) {
            ...
            if (tracked != null) {
                if (tracked.IsNew) {
                 ...
                 throw new Exception(); //CannotRemoveUnattachedEntity();
            }
        }
        ...
        public void Attach(TEntity entity, bool asModified) {
            ...
            MetaType type = this.metaTable.RowType.GetInheritanceType(entity.GetType());
            if (!IsTrackableType(type)) {
                 throw new Exception(); //TypeCouldNotBeTracked(type.Type);
            }
            if (asModified) {
                bool canAttach = type.VersionMember != null || !type.HasUpdateCheck;
                if (!canAttach) {
                     throw new Exception(); //CannotAttachAsModifiedWithoutOriginalState();
                }
            }
            TrackedObject tracked = this.Context.Services.ChangeTracker.GetTrackedObject(entity);
            if (tracked == null || tracked.IsWeaklyTracked) {
                ...
                if (this.Context.Services.InsertLookupCachedObject(type, entity) != entity) {
                    throw new DuplicateKeyException(entity, "CantAddAlreadyExistingKey"/*Strings.CantAddAlreadyExistingKey*/);
                }
                tracked.InitializeDeferredLoaders();
            }
            else {
                 throw new Exception(); //CannotAttachAlreadyExistingEntity();
            }
        }
        ...
        public void Attach(TEntity entity, TEntity original) {
            ...
            if (original == null) {
                 throw new Exception(); //ArgumentNull("original");
            }
            if (entity.GetType() != original.GetType()) {
                 throw new Exception(); //OriginalEntityIsWrongType();
            }
            CheckReadOnly();
            context.CheckNotInSubmitChanges();
            context.VerifyTrackingEnabled();
        ...
        private void CheckReadOnly() {
            if (this.IsReadOnly) {
                 throw new Exception(); //CannotPerformCUDOnReadOnlyTable(ToString());
            }
        }
