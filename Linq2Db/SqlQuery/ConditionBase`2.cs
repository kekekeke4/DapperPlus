...
namespace LinqToDB.SqlQuery
{
		...
		where T1 : ConditionBase<T1,T2>
	{
		public class Expr_
		{
			...
			{
				private readonly Expr_ _expr;
				private readonly SqlPredicate.Operator _op;
				...
				public T2 Expr(ISqlExpression expr)
				{
					return _expr.Add(new SqlPredicate.ExprExpr(_expr._expr, _op, expr));
				}
				...
				public T2 SubQuery(SelectQuery selectQuery)
				{
					return Expr(selectQuery);
				}

				public T2 Value(object value)
				{
					return Expr(new SqlValue(value));
				}

				public T2 All(SelectQuery subQuery)
				{
					return Expr(SqlFunction.CreateAll(subQuery));
				}
			...
			{
				return Add(new SqlPredicate.Like(_expr, false, expression, escape));
			}
			...
			{
				return Like(new SqlValue(expression), null);
			}
			...
			{
				return Add(new SqlPredicate.Between(_expr, false, expr1, expr2));
			}

			public T2 NotBetween(ISqlExpression expr1, ISqlExpression expr2)
			{
				return Add(new SqlPredicate.Between(_expr, true, expr1, expr2));
			}
			...
			public T2 In(SelectQuery subQuery)
			{
				return Add(new SqlPredicate.InSubQuery(_expr, false, subQuery));
			}

			public T2 NotIn(SelectQuery subQuery)
			{
				return Add(new SqlPredicate.InSubQuery(_expr, true, subQuery));
			}
			...
			public T2 In(params object[] exprs)
			{
				return Add(CreateInList(false, exprs));
			}

			public T2 NotIn(params object[] exprs)
			{
				return Add(CreateInList(true, exprs));
			}
		...
		protected abstract SqlSearchCondition Search
		{
			get;
		}
		...
		public Expr_ Expr(ISqlExpression expr)
		{
			return new Expr_(this, false, expr);
		}
		...
		public Expr_ SubQuery(SelectQuery selectQuery)
		{
			return Expr(selectQuery);
		}

		public Expr_ Value(object value)
		{
			return Expr(new SqlValue(value));
		}
