...
namespace LinqToDB.SqlQuery
{
	...
	class SelectQueryOptimizer
	{
						...
						case QueryElementType.LikePredicate:
							{
								...
								if (expr.Escape != null && dic.TryGetValue(expr.Escape, out ex))
								{
									expr.Escape = ex;
								}
								break;
		...
		void OptimizeUnions()
		{
			var isAllUnion = QueryVisitor.Find(_selectQuery, ne => ne is SqlUnion nu && nu.IsAll);
			var isNotAllUnion = QueryVisitor.Find(_selectQuery, ne => ne is SqlUnion nu && !nu.IsAll);
			...
			if (exprs.Count > 0)
			{
				_selectQuery.Walk(new WalkOptions { ProcessParent = true }, expr => exprs.TryGetValue(expr, out var e) ? e : expr);
			}
		}
		...
		bool CorrectCrossJoinQuery(SelectQuery query)
		{
			...
			else
			{
				...
				var toReplace = foundFields.ToDictionary(f => f, f => subQuery.Select.Columns[subQuery.Select.Add(f)] as ISqlExpression);
			...
			bool optimizeColumns)
		{
			...
			var isColumnsOK = (allColumns && !query.Select.Columns.Any(c => IsAggregationFunction(c.Expression))) || !query.Select.Columns.Any(c => CheckColumn(c, c.Expression, query, optimizeValues, optimizeColumns));
			if (!isColumnsOK)
			{
				return childSource;
			}
			...
			((ISqlExpressionWalkable)top).Walk(new WalkOptions(), expr => map.TryGetValue(expr, out var fld) ? fld : expr);
