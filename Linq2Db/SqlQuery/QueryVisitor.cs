...
namespace LinqToDB.SqlQuery
{
	public class QueryVisitor
	{
		...
		private readonly Dictionary<IQueryElement, IQueryElement> _visitedElements = new Dictionary<IQueryElement, IQueryElement>();
		private bool _all;
		private Func<IQueryElement, bool> _action1;
		private Action<IQueryElement> _action2;
		...
		private void Visit1(IQueryElement element)
		{
		...
		private void Visit1X<T>(IEnumerable<T> elements) where T : IQueryElement
		{
			if (elements == null)
			{
				return;
		...
		private void Visit1X(SelectQuery q)
		{
			Visit1(q.Select);
			Visit1(q.From);
			Visit1(q.Where);
		...
		private void Visit1X(SqlOrderByClause element)
		{
		...
		private void Visit1X(SqlGroupByClause element)
		{
			foreach (var i in element.Items)
			{
				Visit1(i);
		...
		private void Visit1X(SqlFromClause element)
		{
			foreach (var t in element.Tables)
			{
				Visit1(t);
			}
		}

		private void Visit1X(SqlSelectClause sc)
		{
			Visit1(sc.TakeValue);
			Visit1(sc.SkipValue);
		...
		private void Visit1X(SqlUpdateClause sc)
		{
			if (sc.Table != null)
			{
				Visit1(sc.Table);
			...
			foreach (var c in sc.Keys.ToArray())
			{
				Visit1(c);
			}
		}

		private void Visit1X(CteClause sc)
		{
		...
		private void Visit1X(SqlInsertClause sc)
		{
			if (sc.Into != null)
			{
				Visit1(sc.Into);
		...
		private void Visit1X(SqlPredicate.InList p)
		{
			Visit1(p.Expr1);
			foreach (var value in p.Values) Visit1(value);
		}

		private void Visit1X(SqlSearchCondition element)
		{
			foreach (var c in element.Conditions) Visit1(c);
		}

		private void Visit1X(SqlTableSource table)
		{
			Visit1(table.Source);
			foreach (var j in table.Joins) Visit1(j);
		}

		private void Visit1X(SqlTable table)
		{
			Visit1(table.All);
		...
		private void Visit1X(SqlWithClause element)
		{
			foreach (var clause in element.Clauses) Visit1(clause);
		}

		private void Visit1X(SqlCteTable table)
		{
			Visit1(table.All);
		...
		private void Visit1X(SqlRawSqlTable table)
		{
			Visit1(table.All);
			foreach (var field in table.Fields.Values)
			{
		...
		private void Visit1X(SqlExpression element)
		{
			foreach (var v in element.Parameters)
			{
				Visit1(v);
			}
		}

		private void Visit1X(SqlFunction element)
		{
		...
		private void Visit2(IQueryElement element)
		{
		...
		private void Visit2X<T>(IEnumerable<T> elements)
		...
		private void Visit2X(SelectQuery q)
		{
			Visit2(q.Select);
		...
		private void Visit2X(SqlOrderByClause element)
		{
			foreach (var i in element.Items)
			{
				Visit2(i);
		...
		private void Visit2X(SqlGroupByClause element)
		{
			foreach (var i in element.Items)
			{
				Visit2(i);
		...
		private void Visit2X(SqlFromClause element)
		{
			foreach (var t in element.Tables)
			{
				Visit2(t);
		...
		private void Visit2X(SqlSelectClause sc)
		{
			Visit2(sc.TakeValue);
			Visit2(sc.SkipValue);
			foreach (var c in sc.Columns.ToArray())
		...
		private void Visit2X(SqlUpdateClause sc)
		{
			if (sc.Table != null)
			{
				Visit2(sc.Table);
		...
		private void Visit2X(CteClause sc)
		{
			foreach (var c in sc.Fields)
			{
				Visit2(c);
		...
		private void Visit2X(SqlInsertClause sc)
		{
			if (sc.Into != null)
			{
				Visit2(sc.Into);
		...
		private void Visit2X(SqlPredicate.InList p)
		{
			Visit2(p.Expr1);
		...
		private void Visit2X(SqlSearchCondition element)
		{
		...
		private void Visit2X(SqlTableSource table)
		{
			Visit2(table.Source);
		...
		private void Visit2X(SqlTable table)
		{
			Visit2(table.All);
			foreach (var field in table.Fields.Values)
			{
		...
		private void Visit2X(SqlRawSqlTable table)
		{
			Visit2(table.All);
			foreach (var field in table.Fields.Values)
			{
		...
		private void Visit2X(SqlWithClause element)
		{
		...
		private void Visit2X(SqlCteTable table)
		{
			Visit2(table.All);
			foreach (var field in table.Fields.Values)
			{
		...
		private void Visit2X(SqlExpression element)
		{
			foreach (var v in element.Parameters)
			{
				Visit2(v);
		...
		private void Visit2X(SqlFunction element)
		{
			foreach (var p in element.Parameters)
			{
				Visit2(p);
		...
		private static IQueryElement Find<T>(IEnumerable<T> arr, Func<IQueryElement, bool> find) where T : class, IQueryElement
		{
			if (arr == null)
			{
				return null;
		...
		private static IQueryElement FindX(SqlSearchCondition sc, Func<IQueryElement, bool> find)
		{
		...
		public static IQueryElement Find(IQueryElement element, Func<IQueryElement, bool> find)
		{
			...
			{
				case QueryElementType.SqlFunction:
					return Find(((SqlFunction)element).Parameters, find);
				case QueryElementType.SqlExpression:
					return Find(((SqlExpression)element).Parameters, find);
				case QueryElementType.Column:
					return Find(((SqlColumn)element).Expression, find);
				case QueryElementType.SearchCondition:
					return FindX((SqlSearchCondition)element, find);
				case QueryElementType.Condition:
					return Find(((SqlCondition)element).Predicate, find);
				case QueryElementType.ExprPredicate:
					return Find(((SqlPredicate.Expr)element).Expr1, find);
				case QueryElementType.NotExprPredicate:
					return Find(((SqlPredicate.NotExpr)element).Expr1, find);
				case QueryElementType.IsNullPredicate:
					return Find(((SqlPredicate.IsNull)element).Expr1, find);
				case QueryElementType.FromClause:
					return Find(((SqlFromClause)element).Tables, find);
				case QueryElementType.WhereClause:
					return Find(((SqlWhereClause)element).SearchCondition, find);
				case QueryElementType.GroupByClause:
					return Find(((SqlGroupByClause)element).Items, find);
				case QueryElementType.OrderByClause:
					return Find(((SqlOrderByClause)element).Items, find);
				case QueryElementType.OrderByItem:
					return Find(((SqlOrderByItem)element).Expression, find);
				case QueryElementType.Union:
					return Find(((SqlUnion)element).SelectQuery, find);
				case QueryElementType.FuncLikePredicate:
					return Find(((SqlPredicate.FuncLike)element).Function, find);
				case QueryElementType.SqlBinaryExpression:
					{
						return
				...
				case QueryElementType.SqlBinaryExpression:
					{
						...
						if (expr1 != null && !ReferenceEquals(expr1, bexpr.Expr1) || expr2 != null && !ReferenceEquals(expr2, bexpr.Expr2))
						{
				...
				case QueryElementType.SqlTable:
					{
						...
						var targs = table.TableArguments == null || table.TableArguments.Length == 0 ? null : Convert(table.TableArguments, action);
				...
				case QueryElementType.SqlRawSqlTable:
					{
						...
						var fields1 = ToArray(table.Fields);
						var fields2 = Convert(fields1, action, f => new SqlField(f));
						var targs = table.Parameters == null || table.Parameters.Length == 0 ? null : Convert(table.Parameters, action);
						var fe = fields2 != null && !ReferenceEquals(fields1, fields2);
				...
				case QueryElementType.TableSource:
					{
						...
						if (source != null && !ReferenceEquals(source, table.Source) || joins != null && !ReferenceEquals(table.Joins, joins))
						{
				...
				case QueryElementType.JoinedTable:
					{
						...
						if (table != null && !ReferenceEquals(table, join.Table) || cond != null && !ReferenceEquals(cond, join.Condition))
						{
		...
		private T[] Convert<T>(T[] arr, Func<IQueryElement, IQueryElement> action)
		...
		private List<T> ConvertSafe<T>(List<T> list, Func<IQueryElement, IQueryElement> action)
			where T : class, IQueryElement
		{
			return ConvertSafe(list, action, null);
		}

		private List<T> ConvertSafe<T>(List<T> list1, Func<IQueryElement, IQueryElement> action, Clone<T> clone)
			where T : class, IQueryElement
		{
			List<T> list2 = null;
