...
namespace LinqToDB.Common
{
	...
	{
		public static void MakeUniqueNames<T>( IEnumerable<T> items, IEnumerable<string> staticNames,  Func<T, string> nameFunc,
			 Action<T, string> nameSetter, string defaultName = "t", StringComparer comparer = null)
		{
		...
		public static void MakeUniqueNames<T>( IEnumerable<T> items, IEnumerable<string> staticNames,  Func<T, string> nameFunc,
			 Action<T, string> nameSetter,  Func<T, string> defaultName, StringComparer comparer = null)
		{
			if (staticNames != null)
			{
		...
		public static void MakeUniqueNames<T>( IEnumerable<T> items,  Func<string, bool> validatorFunc,  Func<T, string> nameFunc,
			 Action<T, string> nameSetter,  Func<T, string> defaultName, StringComparer comparer = null)
		{
			if (items         == null) throw new ArgumentNullException(nameof(items));
